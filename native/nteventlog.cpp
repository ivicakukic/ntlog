...
#include "org_apache_logging_ntlog_Priority.h"
#include "org_apache_logging_ntlog_NTEventLogAppender.h"
#include <windows.h>
#include <jni.h>
...
WORD getCategory(jint priority) {
  WORD category = 1;
  if (priority >= org_apache_logging_ntlog_Priority_DEBUG_INT) {
      category = 2;
      if (priority >= org_apache_logging_ntlog_Priority_INFO_INT) {
          category = 3;
          if (priority >= org_apache_logging_ntlog_Priority_WARN_INT) {
             category = 4;
             if (priority >= org_apache_logging_ntlog_Priority_ERROR_INT) {
                category = 5;
                if (priority >= org_apache_logging_ntlog_Priority_FATAL_INT) {
                    category = 6;
                }
             }
          }
 ...
 */
JNIEXPORT jint JNICALL Java_org_apache_logging_ntlog_NTEventLogAppender_nativeRegisterEventSource(
...
/*
 * Class:     org_apache_logging_ntlog_NTEventLogAppender
 * Method:    reportEvent
 * Signature: (ILjava/lang/String;I)V
 */
JNIEXPORT void JNICALL Java_org_apache_logging_ntlog_NTEventLogAppender_nativeReportEvent(
 ...
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_org_apache_logging_ntlog_NTEventLogAppender_nativeDeregisterEventSource(
JNIEnv *env, 
jobject java_this, 
jint handle)
{
